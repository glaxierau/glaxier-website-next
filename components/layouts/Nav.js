...
const Nav = () => {
    ...
    const [lists, setLists] = useState([
        ...
        { uuid: 'contact', label: 'Contact', dropDown: [], to: '/contact' },
    ])
    ...
    const NavList = ({ to, label, uuid, dropDownList }) => {
        ...
        const onGettingPosition = () => {
            const el = document.getElementById(`${uuid}`).offsetLeft
            setdbPosition(el - 90)
        }
        let dropStatus = dropDownList.length === 0 ? false : true

        console.log({ dropStatus, isdbOpen })
        return (
            <>
                <Link href={to} >
        ...
        <>
            <nav className="fixed hidden lg:flex z-40 top-0 bg-white w-full h-20 px-28 justify-between ">
                ...
                <div className="navlist flex w-3/5 justify-around items-center">
                    {lists.map(list => <NavList key={list.uuid} to={list.to} label={list.label} uuid={list.uuid} dropDownList={list.dropDown} />)}
