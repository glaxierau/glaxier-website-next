    ...
    return content.map(({ _type, _key }) => {
        switch (_type) {
            case 'content':
                return (
                    <BlockContent
                        blocks={filterArray(content, 'content', _key).text}
                        key={_key}
                    />
                )
            case 'singleImage':
                return (
                    <CustomImage
                        image={filterArray(content, 'singleImage', _key).image}
                        key={_key}
                    />
                )
            case 'link':
                return (
                    <CustomLink
                        link={filterArray(content, 'link', _key).link}
                        key={_key}
                    />
                )
            case 'carousel':
                return (
                    <CustomCarousel
                        images={filterArray(content, 'carousel', _key).carousel}
                        key={_key}
                    />
                )
            default:
...
const CustomLink = ({ link }) => {
    return (
        <iframe
            className="w-full h-[350px] rounded-lg my-10"
            allowFullScreen
            src={link}
        ></iframe>
    )
        ...
        <div className="w-full grid lg:grid-cols-3 grid-cols-2 gap-2 my-5">
            {images.map((i, index) => {
                return (
                    <div
                        key={index}
                        className="relative w-full h-[200px] hover:scale-[1.02] transition-all hover:z-10"
                    >
                        <Image
