...
export default function Blogs(props) {
    ...
    useEffect(() => {
        ...
        if (router.query.tags === '') router.push('/blog')
    })
    ...
    const onSearchingForTag = (e) => {
        ...
        const backToNormalCase = filtered.map((tag) =>
            makeFirstCharUpperCase(tag)
        )
    ...
    const onAddingSelectedTags = (tag) => {
        ...
        // If a couple of tags already exist, we want to add an additional tag
            ...
            router.push({
                pathname: router.pathname,
                query: { ...router.query, tags: queries },
            })
    ...
    const onDeletingSelectedTag = (tag) => {
        ...
        if (router.query.tags === '') router.replace('/blog')
        else {
            router.push({
                pathname: router.pathname,
    ...
    return (
        <>
            <SectionHead title="Blog" description="Blog" />
            {/* ---------- Title ------------ */}
            <div
                className={`lg:w-[75vw] md:w-[95vw] w-[92vw] h-auto mx-auto mt-10`}
            >
                <h1
                    className={`text-[50px] font-extrabold text-purple ${
                        page !== 1 && 'hidden'
                    }`}
                >
                    Blog
                ...
                <p
                    className={`my-5 mb-5 lg:w-[60%] w-full ${
                        page !== 1 && 'hidden'
                    }`}
                >
                    Check out our blogs, you can filter the blogs using our
                    tags.
                </p>
            ...
            <div className="grid lg:grid-cols-[3fr_1fr] grid-cols-1 lg:w-[75vw] md:w-[95vw] w-[92vw] mx-auto gap-10 my-10">
                ...
                <div className="mx-auto py-5 px-5 rounded-lg lg:order-1 -order-1 w-full bg-gray-50 h-max">
                    <div className="flex justify-between text-purple text-xl font-bold justify-left items-center">
                        <h2>Tags:</h2>
                        <TfiArrowCircleDown
                            className={`cursor-pointer hover:scale-[1.1] lg:hidden md:block block transition-all ${
                    ...
                    <section
                        className={`${!openTag ? 'hidden' : 'block'} ${
                            !openTag && 'lg:block md:hidden hidden'
                        }`}
                    >
                        {/* Tags search bar */}
                        <input
                        ...
                        {/* Selected tags */}
                        <section
                            className={`my-4 py-2 transition-all ${
                                router.query.tags ? 'border-b-2' : 'hidden'
                            }`}
                        >
                            ...
                            <p
                                className={`mx-auto items-end text-red w-max px-2 rounded-full  text-[12px] cursor-pointer hover:underline transition-all`}
                                onClick={() => {
                                    setTagChunk(10)
                                    router.push({
                                        pathname: router.pathname,
                        ...
                        {/* List of tags */}
                        <div
                            className={`flex flex-wrap lg:justify-start justify-center items-center`}
                        >
                            {tags
                                ?.sort()
                                .slice(0, tagChunk)
                                .map((tag, index) => {
                                    return (
                                        ...
                                        ${
                                            router?.query?.tags?.includes(tag)
                                                ? 'bg-purple text-white'
                                                : 'bg-white text-gray-500'
                                        }`}
                                            onClick={() =>
                                                onAddingSelectedTags(tag)
                                            }
                                        >
                                            {tag}
                                        </div>
                        ...
                        <section className="flex justify-center items-center mx-2">
                            <div className="flex flex-col lg:my-5 my-2 justify-center items-center">
                                <p
                                    className={`text-center text-purple text-sm px-2 rounded-lg cursor-pointer underline transition-all
                            ${tagChunk === 10 ? 'hidden' : 'flex'}`}
                                    onClick={() => setTagChunk(tagChunk - 10)}
                                >
                                    See Less
                                </p>
                            </div>
